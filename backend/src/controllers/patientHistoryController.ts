...
export const getPatientHistoryById = async (req: Request, res: Response) => {
  try {
    ...
    const entryWithPatient = entry as any; // Type assertion to handle include relationship
...
export const createPatientHistory = async (req: Request, res: Response) => {
  try {
    ...
    await AuditService.logCreate(
      ...
      'PatientHistory',
      entry.id,
      AuditService.cleanSensitiveData(entry),
      `Created patient history entry for: ${entry.patient.firstName} ${entry.patient.lastName}`
    );
