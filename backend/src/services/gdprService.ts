import { PrismaClient, ConsentType, DataRequestType, GDPRAction } from '@prisma/client';
...
export class GDPRService {
  ...
  static async recordConsent(
    ...
    try {
      ...
      await this.logGDPRAction(
        req,
        userId,
        consentData.granted ? GDPRAction.CONSENT_GIVEN : GDPRAction.CONSENT_WITHDRAWN,
        'ConsentRecord',
        consentRecord.id,
        'legitimate_interest',
        `Consent ${consentData.granted ? 'granted' : 'withdrawn'} for ${consentData.consentType}`,
        false,
        consentRecord.id
      );
   ...
   */
  static async getUserConsent(userId: string): Promise<Record<ConsentType, boolean>> {
    try {
      ...
      consents.forEach((consent) => {
        if (!consentStatus.hasOwnProperty(consent.consentType)) {
          consentStatus[consent.consentType as ConsentType] = consent.granted;
        }
      });
   ...
   */
  static async exportUserData(req: Request, userId: string): Promise<DataExportResult> {
    try {
   ...
   */
  private static async collectAllUserData(userId: string): Promise<UserDataExport> {
    try {
   ...
   */
  static async hardDeleteUser(req: Request, userId: string): Promise<{ success: boolean; error?: string }> {
    try {
      const user = await prisma.user.findUnique({
        where: { id: userId },
      });
      ...
      await this.logGDPRAction(
        req,
        userId,
        GDPRAction.DATA_DELETION,
        'User',
        userId,
        'erasure_request',
        'User account and all data permanently deleted per GDPR Article 17',
        false
      );
      ...
      for (const policy of policies) {
        try {
          ...
          if (deletedCount > 0) {
            await this.logGDPRAction(
              systemReq,
              null, // System action, not user-specific
              GDPRAction.DATA_DELETION,
              policy.dataType,
              null,
              policy.legalBasis,
              `Automated cleanup: deleted ${deletedCount} expired ${policy.dataType} records per retention policy`,
              true // This is an automated action
            );
          }
        } catch (error) {
   ...
   */
  private static async cleanupExportFiles(): Promise<void> {
    try {
      const exportDir = path.join(process.cwd(), 'exports');
   ...
   */
  static async hardDeletePatient(req: Request, patientId: string, requestedBy?: string): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      const userId = requestedBy || (req as any).user?.id;
      ...
      await this.logGDPRAction(
        ...
        patientId,
        'erasure_request',
        `Patient medical data permanently deleted per GDPR Article 17. Patient: ${patient.firstName} ${patient.lastName}. All related appointments (${patient.appointments.length}), patient history (${patient.patientHistory.length}), packages (${patient.packages.length}), and payments (${patient.payments.length}) also deleted.`,
        false
      );
   ...
   */
  static async bulkHardDeletePatients(req: Request, patientIds: string[], requestedBy?: string): Promise<{ success: boolean; deletedCount: number; errors: string[] }> {
    const errors: string[] = [];
    let deletedCount = 0;
    const userId = requestedBy || (req as any).user?.id;
      ...
      for (const patient of patients) {
        try {
          ...
          await this.logGDPRAction(
            ...
            patient.id,
            'erasure_request',
            `Bulk deletion - Patient medical data permanently deleted per GDPR Article 17. Patient: ${patient.firstName} ${patient.lastName}. Related data: ${patient.appointments.length} appointments, ${patient.patientHistory.length} history entries, ${patient.packages.length} packages, ${patient.payments.length} payments.`,
            false
          );
   ...
   */
  static async exportPatientData(req: Request, patientId: string): Promise<DataExportResult> {
    try {
      ...
      await this.logGDPRAction(
        ...
        patientId,
        'data_portability',
        `Patient medical data exported for GDPR compliance: ${patient.firstName} ${patient.lastName}`,
        false
      );
