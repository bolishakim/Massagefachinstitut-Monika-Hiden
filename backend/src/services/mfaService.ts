...
export class MFAService {
  private static readonly APP_NAME = 'Medical Center';
   ...
   */
  static async disableMFA(userId: string, token: string): Promise<MFAVerificationResult> {
    try {
      const user = await prisma.user.findUnique({
        where: { id: userId },
   ...
   */
  static async verifyMFALogin(userId: string, token: string): Promise<MFAVerificationResult> {
    try {
      const user = await prisma.user.findUnique({
        where: { id: userId },
        select: { mfaSecret: true, mfaEnabled: true, mfaBackupCodes: true }
