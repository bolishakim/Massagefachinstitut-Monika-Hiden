 ...
 */
export const recordBulkConsent = async (req, res) => {
    try {
        const userId = req.user?.id;
        const data = bulkConsentSchema.parse(req.body);
        const results = await Promise.all(data.consents.map(consent => GDPRService.recordConsent(req, userId, {
 ...
 */
export const getComplianceInfo = async (req, res) => {
    try {
