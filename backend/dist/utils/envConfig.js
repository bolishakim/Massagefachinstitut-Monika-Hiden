...
class EnvironmentValidator {
    ...
    static getBooleanFlag(key, defaultValue) {
        const value = process.env[key];
        if (!value)
            return defaultValue;
        return value.toLowerCase() === 'true';
    }
    static validateJWTSecret(secret, name) {
        if (secret.length < 64) {
    ...
    static validateAndLoad() {
        ...
        const config = {
            ...
            COOKIE_SAME_SITE: this.getOptionalString('COOKIE_SAME_SITE', 'strict'),
            COOKIE_HTTP_ONLY: this.getBooleanFlag('COOKIE_HTTP_ONLY', true),
            // CORS Configuration
            CORS_ORIGIN: this.getOptionalString('CORS_ORIGIN', isProduction ? 'https://your-domain.com' : 'http://localhost:3100'),
            // Rate Limiting
    ...
    static logSecurityStatus(config) {
