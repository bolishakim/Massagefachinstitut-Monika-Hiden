...
function getResourceInfo(url, method) {
...
function getActionType(method, url, statusCode) {
    if (method === 'POST')
        return 'CREATE';
...
function getSensitiveDataTypes(resource, body, query) {
    const sensitiveData = [];
    if (resource === 'Patient') {
...
export const enhancedAuditMiddleware = (req, res, next) => {
...
async function logAuthenticationEvent(req, res, auditData) {
    try {
        let authAction;
        let description;
...
export const patientAccessLogger = async (req, res, next) => {
    if (!req.user)
        return next();
    const startTime = Date.now();
    const originalSend = res.send;
...
function shouldLogPatientAccess(url, method, resource) {
...
async function logComprehensivePatientAccess(params) {
    ...
    try {
        ...
        await prisma.auditLog.create({
            data: {
                userId: req.user.id,
                action: req.method === 'GET' ? 'VIEW_DETAILED' : req.method,
...
async function determineAccessDetails(url, method, resourceId, responseData) {
    let accessType = 'unknown';
    let dataType = 'patient_data';
    let tableName = 'Patient';
    let patientId = resourceId;
    let patientInfo = null;
    let dataTypesAccessed = [];
    let sensitiveDataAccessed = [];
        ...
        if (resourceId) {
            ...
            try {
                ...
                if (patient) {
                    ...
                    if (patient.socialInsuranceNumber)
                        sensitiveDataAccessed.push('social_insurance_number');
                    if (patient.dateOfBirth)
                        sensitiveDataAccessed.push('date_of_birth');
                    if (patient.email)
                        sensitiveDataAccessed.push('email_address');
                    if (patient.phone)
                        sensitiveDataAccessed.push('phone_number');
                }
            }
            catch (error) {
...
function getGDPRAction(method, accessType) {
    switch (method) {
        case 'GET':
            return 'DATA_ACCESS';
        case 'POST':
...
//# sourceMappingURL=enhancedAuditMiddleware.js.map